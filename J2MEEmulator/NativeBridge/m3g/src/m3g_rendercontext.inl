...
static M3Gbool m3gCanDirectRender(const RenderContext *ctx)
{
    ...
    if (surfaceType == SURFACE_IMAGE || surfaceType == SURFACE_MEMORY) {
        return M3G_FALSE;
    }
...
static EGLSurface m3gSelectGLSurface(RenderContext *ctx)
{
    ...
    while (attempts <= 1) {
        ...
        case SURFACE_MEMORY:
            lru->handle =
                m3gCreatePBufferSurface(ctx->target.format,
                                        ctx->bufferBits,
                                        ctx->target.width,
                                        ctx->target.height);
            break;
        ...
        case SURFACE_WINDOW:
            ...
            break;
        default:
            printf("[M3G] selectGLSurface: unknown target type 0x%X\n", (unsigned)ctx->target.type);
            /* Fallback: create PBuffer for unknown type */
            lru->handle =
                m3gCreatePBufferSurface(ctx->target.format,
                                        ctx->bufferBits,
                                        ctx->target.width > 0 ? ctx->target.width : 240,
                                        ctx->target.height > 0 ? ctx->target.height : 320);
            break;
        }
