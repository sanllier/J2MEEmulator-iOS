    ...
    if (clazz->status < CLASS_STATUS_CLINITING) {
        ...
        for (i = 0; i < p->method_used; i++) {
            ...
            if (utf8_equals_c(mi->name, STR_METHOD_CLINIT)) {
                ...
                // Skip clinit if thread is being stopped (JVM shutdown).
                // Executing clinit on a dying thread risks use-after-free
                // when jvm_destroy frees classes/objects concurrently.
