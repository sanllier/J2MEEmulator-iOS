...
public class Figure {
	...
	public Figure(byte[] b) {
		...
		try {
			...
			System.err.println("[micro3d] Error loading data: " + e);
	...
	public Figure(String name) throws IOException {
		...
		try {
			...
			System.err.println("[micro3d] Error loading data from [" + name + "]: " + e);
			throw new RuntimeException(e);
		}
	}

	private static byte[] loadResourceAsBytes(String name) {
		InputStream is = Figure.class.getResourceAsStream(name);
		if (is == null && !name.startsWith("/")) {
			is = Figure.class.getResourceAsStream("/" + name);
		}
		if (is == null) {
	...
	public synchronized final void setPosture(ActionTable actionTable, int action, int frame) {
		...
		final TreeMap<Integer, Integer> dynamic = act.dynamic;
		if (dynamic != null) {
			int iFrame = frame < 0 ? 0 : frame >> 16;
			// Find the greatest key <= iFrame (equivalent to SparseIntArray reverse iteration)
			Map.Entry<Integer, Integer> entry = dynamic.floorEntry(iFrame);
			if (entry != null) {
				currentPattern = entry.getValue();
				applyPattern();
			}
		}
	...
	synchronized FloatBuffer getVertexData() {
		if (data.vertexArray == null) {
			data.vertexArray = BufferUtils.createFloatBuffer(data.vertexArrayCapacity / 4);
		}
	...
	synchronized FloatBuffer getNormalsData() {
		...
		if (data.normalsArray == null) {
			data.normalsArray = BufferUtils.createFloatBuffer(data.vertexArrayCapacity / 4);
		}
