...
class Render {
	private final static FloatBuffer BG_VBO = FloatBuffer.wrap(new float[]{
					-1.0f, -1.0f, 0.0f, 0.0f,
					1.0f, -1.0f, 1.0f, 0.0f,
					-1.0f, 1.0f, 0.0f, 1.0f,
					1.0f, 1.0f, 1.0f, 1.0f
	...
	private int gClipLeft, gClipTop, gClipRight, gClipBottom;
	...
	// Reusable scratch buffers to avoid per-frame allocation (GC pressure in miniJVM)
	// Two float buffers needed: vcBuf + ncBuf used simultaneously in renderPrimitives
	private FloatBuffer scratchFloat1;
	private FloatBuffer scratchFloat2;
	private ByteBuffer scratchByte1;
	private ByteBuffer scratchPixels; // for copy2d / release pixel readback

	private FloatBuffer getScratchFloat1(int minCap) {
		if (scratchFloat1 == null || scratchFloat1.capacity() < minCap) {
			scratchFloat1 = BufferUtils.createFloatBuffer(Math.max(minCap, 8192));
		}
		scratchFloat1.clear();
		return scratchFloat1;
	}

	private FloatBuffer getScratchFloat2(int minCap) {
		if (scratchFloat2 == null || scratchFloat2.capacity() < minCap) {
			scratchFloat2 = BufferUtils.createFloatBuffer(Math.max(minCap, 8192));
		}
		scratchFloat2.clear();
		return scratchFloat2;
	}

	private ByteBuffer getScratchByte(int minCap) {
		if (scratchByte1 == null || scratchByte1.capacity() < minCap) {
			scratchByte1 = BufferUtils.createByteBuffer(Math.max(minCap, 8192));
		}
		scratchByte1.clear();
		return scratchByte1;
	}
	...
	static void checkGlError(String glOperation) {
		...
		if (error != GL_NO_ERROR) {
			String s = "0x" + Integer.toHexString(error);
			System.err.println(TAG + " " + glOperation + ": glError " + s);
	...
	private void init() {
		// Context is created on native side via GLES20.mc3dInit()
	}

	// Supersampling scale for micro3D FBO (1 = native, 2 = 2x, etc.)
	// FBO renders at width*MC3D_SCALE x height*MC3D_SCALE, then downscales on blit.
	// Projection matrices still use canvas size for correct center/aspect mapping.
	// Configured from Swift via system property "micro3d.v3.fbo.scale".
	private static final int MC3D_SCALE;
	static {
		int s = 1;
		String val = System.getProperty("micro3d.v3.fbo.scale");
		if (val != null) try { s = Math.max(1, Integer.parseInt(val)); } catch (Exception e) {}
		MC3D_SCALE = s;
	}
	...
	synchronized void bind(Graphics graphics) {
		...
		// Use canvas dimensions if clip covers the full canvas
		// (getClipWidth/Height may reflect the current clip, not the full canvas)
		// For the first bind, clip == canvas size, so this works correctly.
		if (this.width != width || this.height != height) {
			if (this.width != 0) {
				GLES20.mc3dDestroy();
			}
			int fboW = width * MC3D_SCALE;
			int fboH = height * MC3D_SCALE;
			GLES20.mc3dInit(fboW, fboH);
			GLES20.mc3dBind();
			glViewport(0, 0, fboW, fboH);
			// Default GL state — culling controlled per-submesh by renderModel
			Program.create();
			this.width = width;
			this.height = height;
		...
		// Get clip from Graphics
		clipLeft = graphics.getClipX();
		clipTop = graphics.getClipY();
		clipRight = clipLeft + graphics.getClipWidth();
		clipBottom = clipTop + graphics.getClipHeight();
		gClipLeft = clipLeft;
		gClipTop = clipTop;
		gClipRight = clipRight;
		gClipBottom = clipBottom;

		if (clipLeft == 0 && clipTop == 0 && clipRight == this.width && clipBottom == this.height) {
			glDisable(GL_SCISSOR_TEST);
		} else {
			glEnable(GL_SCISSOR_TEST);
			glScissor(clipLeft, clipTop, clipRight - clipLeft, clipBottom - clipTop);
		}
		// Clear both color and depth — color clear is needed because copy2d() is stubbed
		// (in J2ME-Loader, copy2d copies Canvas pixels as GL background before 3D rendering)
	...
	private void copy2d(boolean preProcess) {
		// Copy J2ME Canvas pixels into a GL texture for 2D/3D compositing.
		// preProcess=true: render Canvas as background BEFORE 3D
		// preProcess=false: overlay Canvas on top of 3D AFTER rendering
		...
		// Read Canvas pixels (BGRA in CGBitmapContext) → convert to RGBA for GL
		int pixSize = width * height * 4;
		if (scratchPixels == null || scratchPixels.capacity() < pixSize) {
			scratchPixels = BufferUtils.createByteBuffer(pixSize);
		}
		scratchPixels.clear();
		ByteBuffer pixBuf = scratchPixels;
		NativeBridge.mc3dReadCanvasPixels(graphics.getNativeContext(),
				BufferUtils.getAddress(pixBuf), width, height);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0,
				GL_RGBA, GL_UNSIGNED_BYTE, pixBuf);

		// Draw fullscreen quad with the Canvas texture
	...
	void renderFigure(Model model, int x, int y, FigureLayout layout, Texture[] textures,
		...
		float[] mvp = MVP_TMP;
		MatrixUtil.multiplyMM(mvp, 0, pm, 0, mvm, 0);
	...
	synchronized void release() {
		bindEglContext();
		stack.clear();
		// TODO: post-copy 2D overlay (copy2d(false) for compositing)
		// Read GL framebuffer pixels and blit to J2ME Graphics
		int cw = gClipRight - gClipLeft;
		int ch = gClipBottom - gClipTop;
		if (cw > 0 && ch > 0) {
			int fboW = width * MC3D_SCALE;
			int fboH = height * MC3D_SCALE;
			int pixSize = fboW * fboH * 4;
			if (scratchPixels == null || scratchPixels.capacity() < pixSize) {
				scratchPixels = BufferUtils.createByteBuffer(pixSize);
			}
			scratchPixels.clear();
			ByteBuffer buf = scratchPixels;
			glReadPixels(0, 0, fboW, fboH, GL_RGBA, GL_UNSIGNED_BYTE, buf);
			buf.rewind();
			// Native blit detects scale from fboW vs canvas width, does box-filter downscale
			NativeBridge.mc3dBlitToGraphics(graphics.getNativeContext(), BufferUtils.getAddress(buf), fboW, fboH);
		}
		releaseEglContext();
	}
	...
	synchronized void flush() {
		...
		try {
			...
			glDisable(GL_BLEND);
			glDepthMask(true);
			// Clear depth after each flush — matches J2ME-Loader behavior.
			// Needed so blended primitives (shadows) in subsequent flushes
			// don't fail depth test against stale depth from previous flush.
	...
	private void renderPrimitives(Texture texture, int command, int numPrimitives,
		...
		switch ((command & 0x7000000)) {
			case PRIMITVE_POINTS: {
				...
				FloatBuffer vcBuf = getScratchFloat1(numPrimitives * 3);
				...
				if ((command & PDATA_COLOR_PER_COMMAND) != 0) {
					...
					ByteBuffer colorBuf = getScratchByte(numPrimitives * 3 * 4);
			...
			case PRIMITVE_LINES: {
				...
				FloatBuffer vcBuf = getScratchFloat1(numPrimitives * 2 * 3);
				for (int i = 0; i < vcLen; i++) {
					vcBuf.put(vertices[i]);
				}
				Program.Color program = Program.color;
				...
				if ((command & PDATA_COLOR_PER_COMMAND) != 0) {
					...
					ByteBuffer colorBuf = getScratchByte(numPrimitives * 2 * 3 * 4);
					for (int i = 0; i < numPrimitives; i++) {
						int color = colors[i];
			...
			case PRIMITVE_TRIANGLES: {
				...
				FloatBuffer vcBuf = getScratchFloat1(vcLen);
				for (int i = 0; i < vcLen; i++) {
					vcBuf.put(vertices[i]);
				}
				vcBuf.flip();
				...
				if ((command & PDATA_COLOR_PER_COMMAND) != 0) {
					...
					ByteBuffer tcBuf = getScratchByte(tcLen);
					...
					ByteBuffer colorBuf = getScratchByte(vcLen);
					for (int i = 0; i < numPrimitives; i++) {
						int color = colors[i];
						byte r = (byte) (color >> 16 & 0xFF);
						byte g = (byte) (color >> 8 & 0xFF);
			...
			case PRIMITVE_QUADS: {
				FloatBuffer vcBuf = getScratchFloat1(numPrimitives * 6 * 3);
				for (int i = 0; i < numPrimitives; i++) {
					int offset = i * 4 * 3;
					int pos = offset;
				...
				switch (command & PDATA_NORMAL_PER_VERTEX) {
					case PDATA_NORMAL_PER_FACE:
						ncBuf = getScratchFloat2(numPrimitives * 6 * 3);
				...
				if ((command & PDATA_COLOR_PER_COMMAND) != 0) {
					MatrixUtil.multiplyMM(MVP_TMP, 0, pm, 0, mvm, 0);
					renderMesh(MVP_TMP, mvm, command, blendEnabled, effect, vcBuf, ncBuf, colors[0]);
				} else if ((command & PDATA_TEXURE_COORD) != 0) {
					ByteBuffer tcBuf = getScratchByte(numPrimitives * 6 * 2)
							;
					for (int i = 0; i < numPrimitives; i++) {
						int offset = i * 4 * 2;
						int pos = offset;
					...
					ByteBuffer colorBuf = getScratchByte(numPrimitives * 6 * 3 * 4)
							;
					for (int i = 0; i < numPrimitives; i++) {
						int color = colors[i];
						byte r = (byte) (color >> 16 & 0xFF);
	...
	private void renderSprites(Texture texture, int command, int numPrimitives, int[] vertices, int[] texCoords, FigureLayout layout, float[] pm, float[] mvm, int blend, boolean blendEnabled) {
		...
		float[] m = new float[16];
		MatrixUtil.multiplyMM(m, 0, pm, 0, mvm, 0);
		float[] vert = new float[8];
		float[] quad = new float[4 * 6];

		FloatBuffer vcBuf = getScratchFloat1(numPrimitives * 6 * 4);
		ByteBuffer tcBuf = getScratchByte(numPrimitives * 6 * 2);
		int pos = 0;
		int texOffset = 0;
		for (int i = 0; i < numPrimitives; i++) {
			...
			vert[6] = vertices[pos++];
			vert[7] = 1.0f;
			MatrixUtil.multiplyMV(vert, 0, m, 0, vert, 4);

			if (numParams != 1) {
				texOffset = i * 8;
			}

			float spriteWidth = texCoords[texOffset];
			float spriteHeight = texCoords[texOffset + 1];
			...
			switch (texCoords[texOffset + 7]) {
				case Graphics3D.POINT_SPRITE_LOCAL_SIZE | Graphics3D.POINT_SPRITE_PERSPECTIVE:
					halfWidth = spriteWidth * pm[0] * 0.5f;
					halfHeight = spriteHeight * pm[5] * 0.5f;
					break;
				case Graphics3D.POINT_SPRITE_PIXEL_SIZE | Graphics3D.POINT_SPRITE_PERSPECTIVE:
					if (layout.settingIndex <= Graphics3D.COMMAND_PARALLEL_SIZE) {
						halfWidth = spriteWidth / this.width;
						halfHeight = spriteHeight / this.height;
					} else {
						halfWidth = spriteWidth / this.width * layout.near;
						halfHeight = spriteHeight / this.height * layout.near;
					}
					break;
				case Graphics3D.POINT_SPRITE_LOCAL_SIZE | Graphics3D.POINT_SPRITE_NO_PERS:
					if (layout.settingIndex <= Graphics3D.COMMAND_PARALLEL_SIZE) {
						...
						halfWidth = spriteWidth * pm[0] / near * 0.5f * vert[3];
						halfHeight = spriteHeight * pm[5] / near * 0.5f * vert[3];
					}
					break;
				case Graphics3D.POINT_SPRITE_PIXEL_SIZE | Graphics3D.POINT_SPRITE_NO_PERS:
					halfWidth = spriteWidth / this.width * vert[3];
					halfHeight = spriteHeight / this.height * vert[3];
					break;
				default:
	...
	private void renderMesh(float[] mvp, float[] mv, int command, boolean blendEnabled,
		...
		int vertCount = vertices.limit() / 3;
		vertices.rewind();
		...
		for (int i = 1; i < cmds.length; ) {
			...
			switch (cmd & 0xFF000000) {
				...
				case COMMAND_CENTER:
					...
					break;
				case COMMAND_CLIP: {
					int newL = Math.max(clipLeft, cmds[i++]);
					int newT = Math.max(clipTop, cmds[i++]);
					int newR = Math.min(clipRight, cmds[i++]);
					int newB = Math.min(clipBottom, cmds[i++]);
					clipLeft = newL;
					clipTop = newT;
					clipRight = newR;
					clipBottom = newB;
