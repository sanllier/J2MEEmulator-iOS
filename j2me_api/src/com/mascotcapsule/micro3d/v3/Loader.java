...
class Loader {
	private static final String TAG = "micro3d";
	...
	static Model loadMbacData(byte[] bytes) throws IOException {
		...
		// Initialize vertices buffer with rest-pose data from originalVertices.
		// This ensures correct rendering even before setPosture() is called
		// (e.g. static models, or first frame before animation starts).
		data.originalVertices.rewind();
		data.vertices.rewind();
		int copyCount = Math.min(data.originalVertices.remaining(), data.vertices.remaining());
		for (int vi = 0; vi < copyCount; vi++) {
			data.vertices.put(data.originalVertices.get());
		}
		data.originalVertices.rewind();
		data.vertices.rewind();
		...
		if (normalFormat != 0) {
			...
			data.normals = BufferUtils.createFloatBuffer(len);
			data.normals.put(--len, 1.0f);
			// Initialize normals with original data
			normals.rewind();
			data.normals.rewind();
			int ncopy = Math.min(normals.remaining(), data.normals.remaining());
			for (int ni = 0; ni < ncopy; ni++) {
				data.normals.put(normals.get());
			}
			data.normals.rewind();
		}
		loader.clearCache();
		...
		if (available > 0) {
			System.err.println("[" + TAG + "] Uninterpreted bytes in MBAC (" + available + ", v=" + version);
		}
	...
	static Action[] loadMtraData(byte[] bytes) throws IOException {
		...
		if (transTypeCounts[7] != 0) {
			...
			System.out.println("[" + TAG + "] ActTableData: transTypeCounts[7] = " + transTypeCounts[7]);
		}
		//noinspection unused
		...
		if (available > 0) {
			System.err.println("[" + TAG + "] ActTableData: uninterpreted bytes in MTRA");
		}
	...
	private void readPolyC(Model data, int numVertex, int numColor, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) System.out.println("[" + TAG + "] PolyC unknownByte = " + unknownByte);
	...
	private void readPolyV3(Model data, int numVertex, int numTriangles) throws IOException {
		...
		if (unknownByte != 0) System.out.println("[" + TAG + "] PolyT v3: unknownByte = " + unknownByte);
	...
	private int readUBits(int size) throws IOException {
		if (size > 25) {
			System.err.println("[" + TAG + "] readUBits(size=" + size + ')');
