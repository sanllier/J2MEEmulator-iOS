...
class Utils {
	...
	// Native methods use Buffer address (long) to access raw data directly
	// Pass capacity for bounds checking in native code
	private static native void fillBuffer(long bufAddr, int bufCap, long vertAddr, int vertCap, int[] indices);

	static void fillBuffer(FloatBuffer buffer, FloatBuffer vertices, int[] indices) {
		fillBuffer(BufferUtils.getAddress(buffer), buffer.capacity(),
				   BufferUtils.getAddress(vertices), vertices.capacity(), indices);
	}

	static native void glReadPixels(int x, int y, int width, int height, int[] pixelBuffer);

	private static native void transform(long srcVertAddr, long dstVertAddr,
								 long srcNormAddr, long dstNormAddr,
								 long bonesAddr, int bonesLen, float[] actionMatrices);
	...
	static void transform(FloatBuffer srcVertices, FloatBuffer dstVertices,
		...
		// Each bone = 12 floats (4x3 matrix) + 2 ints (parent, length) = 56 bytes
		int BONE_SIZE = (12 + 2) * 4; // same as sizeof(Bone) in C
		int bonesLen = boneMatrices != null ? boneMatrices.capacity() / BONE_SIZE : 0;
		transform(BufferUtils.getAddress(srcVertices), BufferUtils.getAddress(dstVertices),
				  BufferUtils.getAddress(srcNormals),
				  BufferUtils.getAddress(dstNormals),
				  BufferUtils.getAddress(boneMatrices),
				  bonesLen, actionMatrices);
	}
}
