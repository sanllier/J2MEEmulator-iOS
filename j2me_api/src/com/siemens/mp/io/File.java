...
public class File {
	...
	private static final HashMap<Integer, RandomAccessFile> OPENED_FILES = new HashMap<>();
	...
	public static int spaceAvailable() throws IOException {
		return (int) new java.io.File(System.getProperty("app.save.root")).getUsableSpace();
	}
	...
	private static java.io.File getFile(String fileName) {
		...
		if (colon == -1) {
			return new java.io.File(System.getProperty("app.save.root") + "/" + fileName);
		} else {
