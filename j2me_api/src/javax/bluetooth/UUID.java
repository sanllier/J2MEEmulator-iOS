...
public class UUID {
	...
	public UUID(String uuidValue, boolean shortUUID) {
		if (uuidValue == null) throw new NullPointerException("UUID value is null");
		int length = uuidValue.length();
		if (length == 0) throw new IllegalArgumentException("UUID value is empty");

		if (shortUUID) {
			if (length > 8) throw new IllegalArgumentException("UUID value is too long");
			uuid = java.util.UUID.fromString(
				JSRToJavaString("00000000".substring(length) + uuidValue + SHORT_UUID_BASE));
		} else {
			if (length > 32) throw new IllegalArgumentException("UUID value is too long");
			uuid = java.util.UUID.fromString(
				JSRToJavaString("00000000000000000000000000000000".substring(length) + uuidValue));
		}
	}
