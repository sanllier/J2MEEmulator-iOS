...
public class Graphics3D {
	...
	public void bindTarget(java.lang.Object target, boolean depth, int flags) {
		...
		if (currentTarget != null) {
			// Auto-release if previous bind was not released (e.g., due to exception)
			releaseTarget();
		}
		if (target == null) {
		...
		if (target instanceof Graphics) {
			...
			nativeCtx = finalG.getNativeContext();
			final int width = finalG.getCanvasWidth();
			final int height = finalG.getCanvasHeight();
					...
					new M3gRunnable() {
						...
						public void doRun() {
							_releaseGraphics(handle,
									0, iIsImageTarget, iIsProperRenderer, nativeCtx);
						}
					});
